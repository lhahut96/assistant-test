...
def get_sections_from_category(category_id):
    """Get all sections from a specific category ID"""
    url = f"https://support.optisigns.com/api/v2/help_center/en-us/categories/{category_id}/sections?sort_by=position&sort_order=desc&per_page=100"
...
def save_article_as_markdown(article, output_dir="articles"):
    ...
    markdown_content = html_to_markdown(article.get('body', ''))
...
def get_articles_from_section(section_id):
    """Get all articles from a specific section ID with full content"""
    url = f"https://support.optisigns.com/api/v2/help_center/en-us/sections/{section_id}/articles?sort_by=position&sort_order=desc&per_page=100"
...
def get_sections_from_categories_concurrent(category_ids):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        future_to_category = {
            executor.submit(get_sections_from_category, category_id): category_id 
...
def get_articles_from_sections_concurrent(sections):
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        ...
        future_to_section = {
            executor.submit(get_articles_from_section, section['id']): section 
            for section in sections
        }
...
def main():
    ...
    all_sections = get_sections_from_categories_concurrent(CATEGORY_IDS)
    
    print("-" * 50)
    print(f"Total sections found: {len(all_sections)}")

    # Get all articles from sections concurrently
    all_articles = get_articles_from_sections_concurrent(all_sections)
    ...
    with open("sections_data.json", "w") as f:
        json.dump(all_sections, f, indent=2)

    with open("articles_data.json", "w") as f:
        json.dump(all_articles, f, indent=2)

    print(f"Detailed section data saved to 'sections_data.json'")
    print(f"Detailed article data saved to 'articles_data.json'")
